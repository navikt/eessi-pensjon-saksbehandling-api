...
then
  echo "Setter aad_b2c_clientid_password"
    export loginservice.agent.password=$(cat /var/run/secrets/nais.io/appsecrets/aad_b2c_clientid_password)
fi
...
then
  echo "Setter aad_b2c_clientid_username"
    export loginservice.agent.name=$(cat /var/run/secrets/nais.io/appsecrets/aad_b2c_clientid_username)
fi
